...
Below is a canonical, comprehensive plan aimed at producing whitepaper-grade evidence that local SSD tiering + KV reuse improves runtime inference on DGX Spark (GB10) with Samsung PM9E1 4TB M.2 2242, using Dynamo + TRT-LLM + KVBM as the serving stack. It is structured as two tracks: Plumbing (make the platform provably capable) and Workload shaping (make the loadgen compelling and high-signal).
...
NVIDIA's ICMS/ICMSP framing explicitly treats KV cache as latency-critical but ephemeral, and discusses a tiered hierarchy spanning GPU HBM, host memory, local SSDs, and shared flash tiers, orchestrated by software like Dynamo + KV managers + NIXL. ([NVIDIA Developer][1]) That is the north star; we will start with local NVMe on DGX Spark and keep the plan structurally compatible with future shared storage phases.
...
- SSD under test: Samsung PM9E1 4TB M.2 22x42, PCIe Gen5; Samsung positions it as complementing DGX Spark and quotes read/write capability. ([Samsung Semiconductor Global][2])
...
This model list is taken from NVIDIA's DGX Spark TRT-LLM "Model Support Matrix," updated 01/02/2026. ([NVIDIA Build][3])
...
- Latency at fixed feature set (for example, p95/p99 TTFT improves meaningfully), and/or
- Feature set at fixed latency (for example, higher concurrency or longer context meets the same SLO).
...
Implementation requirement: a single config/flag that selects modes and writes the chosen mode into `manifest.json`.
...
This matters because your eviction/thrash scenarios may otherwise fail to push meaningful disk traffic.
...
These options exist because disk offload can fail on some filesystems; docs mention fallbacks and O_DIRECT disable as troubleshooting steps. ([NVIDIA Docs][4])
...
DGX Spark is explicitly positioned for large models locally (up to ~200B parameters in FP4) and even larger with two systems. NVIDIA's TRT-LLM Spark playbook lists supported quantized larger models (70B NVFP4; GPT-OSS-120B MXFP4). ([NVIDIA Build][3])
...
- C: Offline RAG summarization (long prefill)
...
- Run (C) on 120B across B1/B2

That gives:

- a correctness foundation,
- near-edge relevance,
- and at least one pressure model where SSD tier should matter.
...
4. Tradeoffs: identify regimes where SSD hurts (for example, extra I/O overhead) vs helps.
...
If we follow the plan strictly, the shortest path to compelling results is:
...
3. Implement two workloads first:
   ...
   - Long-context RAG summarization (pressure signal)
...
Samsung explicitly positions PM9E1 as a compact, AI-optimized PCIe Gen5 4TB M.2 22x42 SSD that complements the GB10-powered DGX Spark, and quotes high sequential performance and power-efficiency claims. ([Samsung Semiconductor Global][2]) We should treat those as vendor assertions and focus our evidence on runtime inference outcomes (TTFT, tail latency, max concurrency/context under SLO) that directly match the project goal.
